    ...
    if(!empty($_FILES['file']['name']) && in_array($_FILES['file']['type'], $excelMimes)){ 
        ...
        if(is_uploaded_file($_FILES['file']['tmp_name'])){ 
            ...
            foreach($worksheet_arr as $row){ 
                ...
                $prevQuery = "SELECT Reference_Number FROM aftab_main WHERE Reference_Number = '".$Reference_Number."'"; 
                $prevResult = $db->query($prevQuery); 
                 
                if($prevResult->num_rows > 0){ 
                    // Update member data in the database 
                    $db->query("UPDATE aftab_main SET Reference_Number = '".$Reference_Number."', Remitter_Name = '".$Remitter_Name."', Remitter_Address = '".$Remitter_Address."', Remitter_Country = '".$Remitter_Country."', Paying_Amount = '".$Paying_Amount."', Beneficiary_Name = '".$Beneficiary_Name."', Beneficiary_ID_Number = '".$Beneficiary_ID_Number."', ENTERED_DATE = '".$ENTERED_DATE."', BRANCH_CODE = '".$BRANCH_CODE."', ENTEREDBY = '".$ENTEREDBY."' WHERE Reference_Number = '".$Reference_Number."'"); 
                }else{ 
                    // Insert member data in the database 
                    $db->query("INSERT INTO aftab_main (Reference_Number, Remitter_Name, Remitter_Address, Remitter_Country, Paying_Amount, Beneficiary_Name, Beneficiary_ID_Number, ENTERED_DATE, BRANCH_CODE, ENTEREDBY) 
...
header("Location: index_ace.php".$qstring); 
